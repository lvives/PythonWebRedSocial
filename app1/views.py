        ...
        for comentarioInfo in comentariosPub:
            ...
            for r in respuestas:
                lista_respuestas.append({
                    'autor': f"{r.autoCom.first_name} {r.autoCom.last_name}",
...
def publicarRespuestaComentario(request):
    ...
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            # 2) EXTRAER Y VALIDAR
            texto_respuesta = (data.get('texto_respuesta') or '').strip()  # en Python se usa .strip()
            id_comentario = data.get('comentario_id')
            id_publicacion = data.get('publicacion_id')

            if not texto_respuesta:
                return JsonResponse({'status': 'error', 'detalle': 'Texto vacío'}, status=400)
            if not id_comentario or not id_publicacion:
                return JsonResponse({'status': 'error', 'detalle': 'Faltan IDs'}, status=400)
            if not request.user.is_authenticated:
                return JsonResponse({'status': 'error', 'detalle': 'Usuario no autenticado'}, status=401)

            # 3) OBTENER OBJETOS (usa tus modelos en minúscula)
            comentario_padre = comentario.objects.get(pk=id_comentario)
            pub_obj = publicacion.objects.get(pk=id_publicacion)

            # 4) CREAR RESPUESTA (usa tus campos: pubRel, autoCom, descripcion, respuesta_a)
            comentario.objects.create(
                descripcion=texto_respuesta,
                pubRel=pub_obj,
                autoCom=request.user,
                respuesta_a=comentario_padre
            )
