function cargarPub(idPub)
{
    ...
    .then(data => {
        ...
        for (comenInfo of data.datosComentario)
        {
            ...
            if (Array.isArray(comenInfo.respuestas) && comenInfo.respuestas.length > 0) {
                let respuestasHTML = `<div class="mt-2 ms-4">`;
                comenInfo.respuestas.forEach(resp => {
                    respuestasHTML += `
                        <div class="respuesta p-2 my-2 border-start border-3 border-primary rounded-1 bg-light-subtle">
                            <strong>${resp.autor}</strong><br>
                            <p class="mb-1">${resp.contenido}</p>
                            <small class="text-muted">${resp.fecha ?? ""}</small>
                        </div>
                    `;
                });
                respuestasHTML += `</div>`;
                bloqueComentario += respuestasHTML;
            }
        ...
        <div class="form-respuesta mt-2">
            ...
            <div class="d-flex gap-2 mt-2">
                ...
                <button class="btn btn-outline-secondary btn-sm" 
                        onclick="mostrarFormularioRespuesta(${idComentario}, ${idPublicacion})">
                    Cancelar
                </button>
            </div>
        </div>
...
function enviarRespuesta(idComentario, idPublicacion) {
    ...
    const caja = document.getElementById(`respuesta-${idComentario}`);
    ...
    .then(res => {
        if (res.status === 'ok') {
            caja.value = '';
            const form = caja.closest('.form-respuesta');
            if (form) form.remove();
            cargarPub(idPublicacion);
        } else {
            alert("No se pudo publicar la respuesta. Inténtalo nuevamente.");
            console.error(res);
        }
    })
    .catch(err => {
